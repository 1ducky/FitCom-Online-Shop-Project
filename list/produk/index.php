...
// Ambil produk yang sesuai user
$stmt = $conn->prepare("SELECT user_id, id, nama_produk, harga, gambar, type, kode_produk, stok, satuan 
                        FROM products 
                        WHERE user_id = ?");
...
<?php endif; ?>
            ...
            <div class="card-body">
              ...
              <p class="card-text text-muted">Rp <?= number_format($row['harga'], 0, ',', '.') ?> / <?= htmlspecialchars($row['satuan']) ?></p>
              <p class="card-text"><small>Stok: <?= (int)$row['stok'] ?></small></p>
              <a href="edit.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-warning">
                <i class="fas fa-edit"></i> Edit
              </a>
              <a href="hapus.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Yakin hapus produk ini?');">
